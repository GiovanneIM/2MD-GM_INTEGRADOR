...
export default function Sessoes({
    ...
    function cancelarSessao(sessao) {
        MySwal.fire({
            scrollbarPadding: false,
            heightAuto: false,
            ...
            didOpen: () => {
                document.getElementById('btnDesmarcarSessao').addEventListener('click', () => {
                    fetch(`http://localhost:3000/api/treinamentos/${sessao.id}/cancelarSessao`, {
                        method: 'DELETE',
                    }).then(res => res.json()
                    ).then(data => {
                        console.log(data);
        ...
        MySwal.fire({
            ...
            didOpen: () => {
                ...
                document.getElementById('btnConcluirSessao').addEventListener('click', () => {
                    fetch(`http://localhost:3000/api/treinamentos/${sessao.id}/atualizarEstadoSessao`, {
                        method: 'POST',
                        headers: {
            ...
            <div className='flex-grow-1 border p-3 d-flex flex-column gap-3 overflow-y-scroll'>
                ...
                {sessoes.map((s) =>
                    <div className='border p-3' key={s.indice}>
                        ...
                        {(criador && s.estado != 'Concluida') ?
                            <div className='d-flex gap-3'>
                                <button className='btn btn-danger' onClick={(e) => cancelarSessao(s)}>
                                ...
                                <button className='btn btn-success' onClick={(e) => concluirSessao(s)}>
